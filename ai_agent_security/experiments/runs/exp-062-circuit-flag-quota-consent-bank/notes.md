...
```bash
python3 -m py_compile experiments/runs/exp-062-circuit-flag-quota-consent-bank/attack.py
$HOME/.local/bin/aicomp validate redteam experiments/runs/exp-062-circuit-flag-quota-consent-bank/attack.py
python3 scripts/validate_submission_bundle.py kaggle-push/exp-062
python3 scripts/build_exp_notebook.py exp-062-circuit-flag-quota-consent-bank --description "Circuit breaker/feature flag/quota broker/consent ledger replay bank with compact Go-Explore fallback."
```

## 提出

1. `kaggle-push/exp-062` の bundle を Kaggle kernels push する。
2. Kaggle UI で version description に `exp-062-circuit-flag-quota-consent-bank` を含めて notebook 自体を提出する。
