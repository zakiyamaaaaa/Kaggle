...
```bash
python3 -m py_compile experiments/runs/exp-053-workflow-dag-taskqueue-checkpoint-idempotency-bank/attack.py
$HOME/.local/bin/aicomp validate redteam experiments/runs/exp-053-workflow-dag-taskqueue-checkpoint-idempotency-bank/attack.py
python3 scripts/validate_submission_bundle.py kaggle-push/exp-053
python3 scripts/build_exp_notebook.py exp-053-workflow-dag-taskqueue-checkpoint-idempotency-bank --description "Workflow DAG/task queue/checkpoint resume/idempotency key replay bank with compact Go-Explore fallback."
```

## 提出

1. `kaggle-push/exp-053` の bundle を Kaggle kernels push する。
2. Kaggle UI で version description に `exp-053-workflow-dag-taskqueue-checkpoint-idempotency-bank` を含めて notebook 自体を提出する。
