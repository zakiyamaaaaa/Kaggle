...
```bash
python3 -m py_compile experiments/runs/exp-074-circuit-flag-quota-consent-bank/attack.py
$HOME/.local/bin/aicomp validate redteam experiments/runs/exp-074-circuit-flag-quota-consent-bank/attack.py
python3 scripts/validate_submission_bundle.py kaggle-push/exp-074
python3 scripts/build_exp_notebook.py exp-074-circuit-flag-quota-consent-bank --description "Circuit breaker/feature flag/quota broker/consent ledger replay bank with compact Go-Explore fallback."
```

## 提出

1. `kaggle-push/exp-074` の bundle を Kaggle kernels push する。
2. Kaggle UI で version description に `exp-074-circuit-flag-quota-consent-bank` を含めて notebook 自体を提出する。
3. スコア反映後に `python3 scripts/exp.py sync` を実行する。

## 自動化実行メモ（2026-09-25）

- main 上の最新実験 `exp-073` の次番として、circuit / feature flag / quota / consent 系 bank を復帰させた `exp-074` を提出可能形で追加。
- exp-073 の workflow / task queue / checkpoint / idempotency テーマを circuit / feature flag / quota / consent テーマに差し替え（exp-072 と同系の bank 構成、ベースは exp-073 系の core replay）。
- replay bank: **1998** 件（ユニーク 1998、最大 3 ターン、最大メッセージ長 144）。
- ローカル検証: `py_compile` OK、`aicomp validate` OK、`validate_submission_bundle.py kaggle-push/exp-074` OK、`build_exp_notebook.py` で notebook 同期 OK。
