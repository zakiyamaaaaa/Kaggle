...
```bash
python3 -m py_compile experiments/runs/exp-075-workflow-dag-taskqueue-checkpoint-idempotency-bank/attack.py
$HOME/.local/bin/aicomp validate redteam experiments/runs/exp-075-workflow-dag-taskqueue-checkpoint-idempotency-bank/attack.py
python3 scripts/validate_submission_bundle.py kaggle-push/exp-075
python3 scripts/build_exp_notebook.py exp-075-workflow-dag-taskqueue-checkpoint-idempotency-bank --description "Workflow DAG/task queue/checkpoint resume/idempotency key replay bank with compact Go-Explore fallback."
```

## 提出

1. `kaggle-push/exp-075` の bundle を Kaggle kernels push する。
2. Kaggle UI で version description に `exp-075-workflow-dag-taskqueue-checkpoint-idempotency-bank` を含めて notebook 自体を提出する。
3. スコア反映後に `python3 scripts/exp.py sync` を実行する。

## 自動化実行メモ（2026-09-26）

- main 上の最新実験 `exp-074` の次番として、workflow DAG / task queue / checkpoint / idempotency 系 bank を復帰させた `exp-075` を提出可能形で追加。
- exp-074 の circuit / feature flag / quota / consent テーマを workflow / task queue / checkpoint / idempotency テーマに差し替え（exp-073 と同系の bank 構成、ベースは exp-074 系の core replay）。
- replay bank: **1998** 件（ユニーク 1998、最大 3 ターン、最大メッセージ長 144）。
- ローカル検証: `py_compile` OK、`aicomp validate` OK、`validate_submission_bundle.py kaggle-push/exp-075` OK、`build_exp_notebook.py` で notebook 同期 OK。
