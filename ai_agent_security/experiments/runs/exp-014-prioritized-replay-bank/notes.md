# exp-014-prioritized-replay-bank

- 名前: prioritized-replay-bank
- Kaggle タグ: `exp-014-prioritized-replay-bank`
...
```bash
python3 -m py_compile experiments/runs/exp-014-prioritized-replay-bank/attack.py
aicomp validate redteam experiments/runs/exp-014-prioritized-replay-bank/attack.py
python3 scripts/validate_submission_bundle.py kaggle-push/exp-014
```
...
```bash
aicomp test redteam experiments/runs/exp-014-prioritized-replay-bank/attack.py \
...
1. `kaggle-push/exp-014` の bundle を Kaggle kernels push する。
2. Kaggle UI で version description に `exp-014-prioritized-replay-bank` を含めて notebook 自体を提出する。
