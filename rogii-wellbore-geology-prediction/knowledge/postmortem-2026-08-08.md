...
初期のlast-value baselineはpublic/private 15.883/14.488だった。最終score 9.216まで大幅に改善できたこと、リークを含む公開6点台コードをそのまま最終候補にしなかったことは成果である。一方、最終順位はメダル圏のすぐ外側だった。private scoreは締切後まで見えないため9.074を事前に知って選ぶことは不可能だったが、最終2枠の1枠をpublic順位に依存しない別系統候補へ割り当てる運用は可能だった。
...
重要な事実は、public best 7.474がprivateでは9.286であり、publicで0.151悪かったComplete-well 7.625がprivateでは全提出中最良の9.074だったことである。private leaderboardのスコア分布に9.074を当てはめると約546位相当で、実際の665位より約119位高い。6,125チームの単純な上位10%線は約613位なので、結果論ではメダル圏相当である。ただし、private 9.074は締切後に判明した値であり、当時これを根拠に選ぶことはできない。事前に実行できたのは、最高publicの1本に加え、ローカルの空間耐性とモデルfamilyの多様性で選ぶhedge枠を1本確保することだった。
...
### 4.1 最終submissionの多様性ルールを実行しなかった

private scoreを締切前に知ることはできないため、Complete-wellを「9.074だから選ぶ」対応は不可能だった。また、コンペ終了後にfinal submissionを変更する通常の手段もない。対応可能だったのは締切前の選択方針である。public上位の7.474と7.539をそのまま残すのではなく、1枠を最高public、もう1枠をローカルの空間耐性とmodel familyの多様性で選ぶhedgeにする計画は既に作っていた。この計画を実行していれば、Complete-wellを2本目に選ぶ合理的な余地があった。
...
このコンペでは、baseline 14点台からfinal 9.216まで進み、リークを含む公開手法を識別する力と再現性のある提出基盤を得た。一方、privateで最良だった9.074の候補はfinalに選ばれず、実順位は665位となった。9.074を事前に知ることは不可能だったが、最高publicと別系統hedgeを1本ずつ選ぶ手順は実行できた。最大の反省は「publicで最も良いモデルを残すこと」と「hiddenで頑健な可能性を持つ異系統モデルを残すこと」を最終2枠で分離しなかった点である。
