...
- 出力準備: `scripts/learned_selector.py` を追加し、全train prefixから再学習してtest形式のローカル予測CSVを生成できるようにした。Kaggle API、外部提出、Notebook再実行は行っていない。
- 2026-07-22 の再実行では、評価済み候補CSVの生成を試みたが、利用可能なローカルPython環境に `scikit-learn` がなく `HistGradientBoostingRegressor` を再現できず停止した。ネットワーク経由の依存追加や外部提出は行わない。評価台帳のRMSE・井戸別p50/p90・実行時間と、再現用スクリプトは保持する。次回は `scikit-learn` が利用可能なローカル実行環境で `learned_selector.py` を実行し、出力CSVの行数・nullを検証する。
