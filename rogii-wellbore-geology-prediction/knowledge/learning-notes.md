...
| 全773井のHGB候補選択（全suffix、5-fold） | RMSE 14.7196（raw）/14.7212（±40 guard） | — | 現行ローカル最良・未提出 |
...
1. 複数windowのNCCとbeamの合意ゲートを、空間平面blendの前段フィルタとして検証する。
2. 773井から候補軌跡を生成し、well-grouped OOFでLightGBM/XGBoost相当のresidual meta-modelを学習する。
3. 低周波トレンド（line/quadratic/smooth oracle）の誤差を、prefix由来の特徴で予測する。
...
## 2026-07-22 selector低周波trend_delta_md_100評価

- 仮説: 既存の候補選択器が持つ最近のprefix傾向を、直近100点のMD-TVT傾きにsuffixのMD距離を掛けた因果的な低周波変位特徴 `trend_delta_md_100` として明示すれば、候補残差の長周期ドリフトを選択器が捉えやすくなる。
- 実装: `scripts/supervised_residual.py` の特徴に `slope_md_100 * md_from_start` を1列追加し、HGB残差selectorの他の候補軌道・固定seed・GroupKFold設定は変更しなかった。HGBのtest予測はclip10 ftの保守guardを使う `scripts/learned_selector.py` に更新した。
- リーク確認: `trend_delta_md_100` は観測 `TVT_input` prefix、MD、水平井のGR/XYZ、typewell decoder、他井のprefix-only空間メタデータだけから作成した。suffix真値 `TVT` は学習ターゲットと評価にのみ使用し、test形式（TVT列なし）でも特徴生成できることを確認した。
- 結果: 全773井・193,250 sampled suffix行（250行/井、固定seed、GroupKFold 5）で候補RMSE 15.1905444、HGB raw 14.6040765、clip10 14.4144578。clip10の井戸別RMSE p50/p90は9.6419717/20.2620221、実行時間305.980秒。既存clip10基準14.6112862より0.1968284改善したため improved とした。fold 1だけは候補14.514515に対しHGB14.521525と悪化しており、全fold集計で採否を判断した。
- 出力: `outputs/submissions/learned_selector_hgb_trend100_clip10.csv` を生成し、14,151行、null 0、ID重複 0、`sample_submission.csv` とID順一致、予測値11593.4342〜12234.2172を確認した。Kaggle API、competition submit、公開Notebook実行は行っていない。
- 判断: 低周波trend特徴付きclip10 selectorをローカル採用候補として保存する。ただしOOFは250行/井の標本評価であり、hidden testの分布差とclip guardの保守性を考慮して、外部提出は保留する。次回はこの特徴を再評価せず、別のselector特徴または軽量な保守guardを1件だけ検討する。

## 2026-07-22 selector低周波trend_delta_md_200評価

- 仮説: 直近100点の傾きから作った `trend_delta_md_100` が改善したため、より長いが全体傾向を残す直近200点のprefix MD-TVT傾きにsuffixのMD距離を掛けた `trend_delta_md_200` を明示すれば、HGB selectorが中期ドリフトを安定して扱える可能性がある。
- 実装: `scripts/supervised_residual.py` に `slope_md_200 * md_from_start` を1列追加し、候補軌道、固定seed、250行/井の抽出、GroupKFold(5)、HGB設定、clip10 guardは固定した。`scripts/learned_selector.py` の既定出力名を `learned_selector_hgb_trend200_clip10.csv` に更新した。
- リーク確認: 特徴は観測 `TVT_input` prefix、suffixで推論時に利用できるMD/GR/XYZ、typewell decoder、他井のprefix-only空間メタデータだけから作成した。suffix真値 `TVT` は学習ターゲットと評価にのみ使用し、test形式（TVT列なし）で特徴生成・出力できることを確認した。
- 結果: 20井・5,000行のsmokeを完走した後、全773井・193,250 sampled suffix行で候補RMSE 15.1905444、HGB raw 14.6191233、clip10 14.4025587、clip10井戸別RMSE p50/p90 9.7752588/20.1807305、elapsed 290.138秒。前回 `trend_delta_md_100 + clip10` の14.4144578より0.0118992改善したため採用候補とした。
- 出力: `outputs/submissions/learned_selector_hgb_trend200_clip10.csv` は14,151行、null 0、ID重複0、`sample_submission.csv` とID順一致、予測値11593.5505〜12234.2172。Kaggle API、competition submit、公開Notebook実行は行っていない。
- 判断: `trend_delta_md_200 + clip10` をローカル採用候補として保存する。ただし評価は250行/井の標本OOFであり、hidden test分布差を考慮して外部提出は保留する。次回はこの特徴を再評価せず、別のselector特徴または軽量guardを1件だけ検討する。

## 2026-07-22 selector低周波trend_delta_z_100評価

- 仮説: MD方向の `trend_delta_md_200` が改善したため、直近100点のprefix Z-TVT傾きにsuffixのZ変位を掛ける `trend_delta_z_100` を追加すれば、坑井の幾何方向に沿う低周波変位をselectorが補足できる可能性がある。
- 実装: `scripts/supervised_residual.py` に `slope_z_100 * z_from_start` を1列追加し、候補軌道、他の因果特徴、固定seed、250行/井の抽出、GroupKFold(5)、HGB設定、clip10 guardは固定した。候補ソースは rejected のため採用しない。
- リーク確認: `trend_delta_z_100` は観測 `TVT_input` prefix、suffixで予測時に利用できるZ、既存のGR/MD/XYZ・typewell decoder・prefix-only空間メタデータだけから作成した。suffix真値 `TVT` は学習ターゲットと評価にのみ使用した。
- 結果: 全773井・193,250 sampled suffix行で候補RMSE 15.1905444、HGB raw 14.6366964、clip10 14.4422103、clip10井戸別RMSE p50/p90 9.7396110/20.4733182、elapsed 313.076秒。採用済み `trend_delta_md_200 + clip10` の14.4025587より0.0396516悪化した。
- 判断: rejected。Z方向の直近傾きは今回のselector残差に有効な追加情報を与えず、既存のMD200特徴と併用しても改善しなかった。submission CSVは生成せず、Kaggle API・competition submit・公開Notebook実行・GitHub pushは行っていない。
- 次の仮説: `trend_delta_md_200 + clip10` を維持し、同じZ100やMD100/MD200を再試行せず、別のprefix-only selector特徴または軽量guardを1件だけ検証する。
