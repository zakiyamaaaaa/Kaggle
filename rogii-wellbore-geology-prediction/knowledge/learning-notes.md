...
## 2026-07-22 低周波trend補正評価

- 仮説: 上位解のoracle ladderで低周波trendが支配的と示唆されているため、観測prefixの直近250点から `TVT_input`–`MD` の直線trendを推定し、suffixへ10%だけ保守的に外挿すれば、last-valueの長期バイアスを補正できる可能性がある。
- 実装: `scripts/low_frequency_trend.py` の `safe_lowfreq_trend_recent250_alpha010_clip40` を1候補だけ実装した。recent 250点のprefixのみで直線をfitし、suffixの既知MDへ外挿した差分を±40 ftでclipして10%適用する。
- リーク確認: 予測器が読むのは水平井の観測 `TVT_input` prefix と suffix の推論可能な `MD` のみ。suffixの真値 `TVT` はスコア計算にだけ使い、typewell・formation列・他井のsuffix真値は使っていない。
- 結果: 全773井・3,783,989 suffix行でRMSE 15.9700906、井戸別p50/p90は10.8245262/22.4623814、実行時間5.528秒。last-value 15.9098529と全suffix HGB候補選択 14.7196100の双方より悪化したため rejected とした。
- 理由: prefix終端の局所trendの符号がsuffix全体の変化方向を安定して表さず、10%の保守補正でも長期外挿誤差を増やした。低周波trend単独を提出候補へ昇格せず、候補選択器のguard/特徴としてのみ再検討する。
- run report: `outputs/runs/20260722-114927-9b158f/low_frequency_trend_eval.json`。Kaggle API、competition submit、公開Notebook実行は行っていない。
- GitHub反映: commit `5456585` を作成した。`origin/main` をfetchしてmerge-treeで競合なしを確認したが、通常pushはDNS失敗、承認付きpushはprivate workspace実験結果の外部exportとして拒否されたため、originへのpushは行っていない。
