...
| 全773井のHGB候補選択（全suffix、5-fold、±20 guard） | RMSE 14.6416 | — | 現行ローカル最良・未提出 |
...
## 2026-07-22 候補選択器の±20 ft guard評価

- 仮説: 全suffixのHGB候補選択はrawの外れ補正を許すと隠れtestで不安定になる可能性がある。既存の±40 ft guardよりも保守的な±20 ftに狭めれば、候補間の大きな誤補正を抑えつつ、候補選択の改善を残せる。
- 実装差分: 候補生成、因果特徴、HGB設定、固定seed、GroupKFold(5)、773井・3,783,989 suffix行を固定し、残差補正のclip幅だけを±20 ftへ変更した。テスト予測生成にも `--guard-clip 20` を追加し、既存の±40候補は維持した。
- リーク確認: 特徴は水平井のGR・MD・XYZ、観測 `TVT_input` prefix、typewellのTVT/GR、他井のprefix-only空間anchorだけを使用した。suffixの真値 `TVT` は学習ターゲットと評価にのみ使い、推論特徴には渡していない。GroupKFoldで井戸単位に分離した。
- 結果: 全773井・3,783,989 suffix行で、候補基準15.5180228、HGB raw 14.7196100、±40 ft 14.7212188に対し、±20 ftはRMSE 14.6416273。井戸別RMSE p50/p90は10.0419285/20.9248332、評価時間688.234秒だった。±40 ftより0.0795915改善し、従来の現行ローカル最良を更新した。
- 出力確認: `outputs/submissions/learned_selector_hgb_clip20.csv` は14,151行で、`sample_submission.csv`とのID順が完全一致、null 0、ID重複0、予測値範囲11592.9542〜12235.2147。Kaggle API、competition submit、公開Notebook実行は行っていない。
- 判断: ±20 ft候補をローカル採用し、±40 ft候補を保守的な比較対象として残す。隠れtest分布差が不明なため、Kaggle提出は行わない。
- 次の仮説: selectorのguard幅探索は±20を新基準として打ち切り、次回は低周波trend特徴を1つだけ追加して、改善が候補選択器に再現するか検証する。
