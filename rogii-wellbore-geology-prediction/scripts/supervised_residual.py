...
def build_features(hw: pd.DataFrame, tw: pd.DataFrame, mod) -> tuple[pd.DataFrame, np.ndarray]:
    known = hw[hw["TVT_input"].notna()]
    unknown = hw[hw["TVT_input"].isna() & hw["TVT"].notna()]
    ...
    # The spatial candidate uses only prefix anchors from other wells when a
    # metadata object is supplied. For this first model, omit it to make the
    # group-wise CV independent of fold construction.
    n = len(unknown)
...
def run(data_root: Path, max_wells: int | None, rows_per_well: int, folds: int) -> dict[str, float]:
    ...
    for i, fp in enumerate(files, 1):
        ...
        if not tw_path.exists():
            continue
        hw = pd.read_csv(fp)
        frame, target = build_features(hw, pd.read_csv(tw_path), mod)
        if frame.empty:
            continue
    ...
    baseline = X["baseline"].to_numpy(float)
    residual = y - baseline
    ...
    for fold, (train_idx, valid_idx) in enumerate(cv.split(X, residual, group), 1):
        ...
        oof[valid_idx] = baseline[valid_idx] + model.predict(X.iloc[valid_idx])
        print(
            f"fold {fold}: baseline={mean_squared_error(y[valid_idx], baseline[valid_idx])**0.5:.6f} "
    ...
    return {
        "method": "group_hgb_residual",
        "rows": int(good.sum()),
        "wells": int(pd.Series(group).nunique()),
        "baseline_rmse": float(mean_squared_error(y[good], baseline[good]) ** 0.5),
...
def main() -> None:
    ...
    parser.add_argument("--rows-per-well", type=int, default=250)
